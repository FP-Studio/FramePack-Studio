...
def load_lora(
    transformer: torch.nn.Module, lora_path: Path, weight_name: str
    ...
    if hasattr(transformer, "peft_config") and adapter_name in transformer.peft_config:
        print(
            f"Adapter '{adapter_name}' already exists. Removing it before loading again."
        )
    ...
    transformer.load_lora_adapter(
        state_dict, network_alphas=None, adapter_name=adapter_name
    )
...
def resolve_expansion_class_name(
    ...
    if fallback_class in fn_mapping:
        print(
            f"Warning: No scale function for '{class_name}'. Falling back to '{fallback_class}'"
        )
...
def set_adapters(
    ...
    if resolved_class_name:
        ...
        print(
            f"Using scale expansion function for model class '{resolved_class_name}' (original: '{transformer_class_name}')"
        )
        final_weights = [
        ...
        print(
            f"Warning: No scale expansion function found for '{transformer_class_name}'. Using raw weights."
        )
