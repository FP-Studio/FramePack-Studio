...
class VideoProcessor:
    def __init__(self, message_manager: MessageManager, settings):
        ...
        self._base_temp_output_dir = (
            self.postprocessed_output_root_dir / "temp_processing"
        )
        self._base_permanent_save_dir = (
            self.postprocessed_output_root_dir / "saved_videos"
        )
    ...
    def tb_process_video_batch(
        self, video_paths: list, pipeline_config: dict, progress=gr.Progress()
    ):
        """
        ...
        try:
            if is_batch_job:
                ...
                batch_output_dir = (
                    self._base_permanent_save_dir / f"batch_process_{timestamp}"
                )
                os.makedirs(batch_output_dir, exist_ok=True)
                self.message_manager.add_message(
                    f"Batch outputs will be saved to: {batch_output_dir}", "SUCCESS"
                )
            ...
            if not operations:
                self.message_manager.add_warning(
                    "No operations were selected for the pipeline. Nothing to do."
                )
                return None

            op_names = [op["name"].replace("_", " ").title() for op in operations]
            self.message_manager.add_message(
                f"🚀 Starting pipeline for {len(video_paths)} videos. Pipeline: {' -> '.join(op_names)}"
            )
            ...
            for i, original_video_path in enumerate(video_paths):
                progress(
                    i / total_videos,
                    desc=f"Video {i + 1}/{total_videos}: {os.path.basename(original_video_path)}",
                )
                self.message_manager.add_message(
                    f"\n--- Processing Video {i + 1}/{total_videos}: {os.path.basename(original_video_path)} ---",
                    "INFO",
                )
                ...
                for op_config in operations:
                    ...
                    self.message_manager.add_message(
                        f"  -> Step: Applying {op_name.replace('_', ' ')}..."
                    )
                    output_path = None
                    try:
                        if op_name == "upscale":
                            output_path = self.tb_upscale_video(
                                current_video_path, **op_params, progress=progress
                            )
                        elif op_name == "frame_adjust":
                            output_path = self.tb_process_frames(
                                current_video_path, **op_params, progress=progress
                            )
                        elif op_name == "filters":
                            output_path = self.tb_apply_filters(
                                current_video_path, **op_params, progress=progress
                            )
                        elif op_name == "loop":
                            output_path = self.tb_create_loop(
                                current_video_path, **op_params, progress=progress
                            )
                        elif op_name == "export":
                            output_path = self.tb_export_video(
                                current_video_path, **op_params, progress=progress
                            )

                        if output_path and os.path.exists(output_path):
                            self.message_manager.add_success(
                                f"  -> Step '{op_name}' completed. Output: {os.path.basename(output_path)}"
                            )
                            if path_to_clean and os.path.exists(path_to_clean):
                                try:
                                    os.remove(path_to_clean)
                                    self.message_manager.add_message(
                                        f"  -> Cleaned intermediate file: {os.path.basename(path_to_clean)}",
                                        "DEBUG",
                                    )
                                except OSError as e:
                                    self.message_manager.add_warning(
                                        f"Could not clean intermediate file {path_to_clean}: {e}"
                                    )
                        ...
                        self.message_manager.add_error(
                            f"An unexpected error occurred during step '{op_name}': {e}"
                        )
                ...
                if not video_failed:
                    ...
                    if is_batch_job:
                        ...
                        final_dest_path = batch_output_dir / os.path.basename(
                            final_temp_path
                        )
                        shutil.copy2(
                            final_temp_path, final_dest_path
                        )  # Use copy2 to keep temp file for UI
                        self.message_manager.add_success(
                            f"--- Successfully processed. Final output saved to: {final_dest_path} ---"
                        )
                        ...
                        if original_autosave_state:
                            final_dest_path = (
                                self._base_permanent_save_dir
                                / os.path.basename(final_temp_path)
                            )
                            shutil.move(
                                final_temp_path, final_dest_path
                            )  # Move, as it's saved permanently
                            self.message_manager.add_success(
                                f"--- Successfully processed. Final output saved to: {final_dest_path} ---"
                            )
                            ...
                            self.message_manager.add_success(
                                f"--- Successfully processed. Final output is in temp folder: {final_temp_path} ---"
                            )
                            last_successful_video_path_for_ui = final_temp_path
                else:
                    self.message_manager.add_warning(
                        f"--- Processing failed for {os.path.basename(original_video_path)} ---"
                    )
                    if path_to_clean and os.path.exists(path_to_clean):
                        try:
                            os.remove(path_to_clean)
                            ...
                            self.message_manager.add_warning(
                                f"Could not clean failed intermediate file {path_to_clean}: {e}"
                            )
    ...
    def _report_ffmpeg_status(self):
        ...
        if self.ffmpeg_source == "Bundled" and self.ffprobe_source == "Bundled":
            self.message_manager.add_message(
                f"Bundled FFmpeg found: {self.ffmpeg_exe}", "SUCCESS"
            )
            self.message_manager.add_message(
                f"Bundled FFprobe found: {self.ffprobe_exe}", "SUCCESS"
            )
            self.message_manager.add_message(
                "All video and audio features are enabled.", "SUCCESS"
            )
            return

        if self.has_ffmpeg:
            self.message_manager.add_message(
                f"FFmpeg found via {self.ffmpeg_source}: {self.ffmpeg_exe}", "SUCCESS"
            )
        else:
            self.message_manager.add_error(
                "Critical: FFmpeg executable could not be found. Most video processing operations will fail. Please try running the setup script."
            )

        if self.has_ffprobe:
            self.message_manager.add_message(
                f"FFprobe found via {self.ffprobe_source}: {self.ffprobe_exe}",
                "SUCCESS",
            )
        else:
            self.message_manager.add_warning(
    ...
    def tb_get_frames_from_folder(self, folder_name: str) -> list:
        ...
        if not os.path.isdir(full_folder_path):
            self.message_manager.add_error(
                f"Cannot load frames: Directory not found at {full_folder_path}"
            )
            return []
        ...
        try:
            ...
            self.message_manager.add_error(
                f"Error reading frames from '{folder_name}': {e}"
            )
            return []
    ...
    def tb_delete_single_frame(self, frame_path_to_delete: str) -> str:
        ...
        if not frame_path_to_delete or not isinstance(frame_path_to_delete, str):
            ...
            self.message_manager.add_error(
                "Could not delete frame: Invalid path provided to processor."
            )
        ...
        try:
            ...
            if os.path.isfile(frame_path_to_delete):
                ...
                self.message_manager.add_error(
                    f"Could not delete frame. File not found: {frame_path_to_delete}"
                )
    ...
    def tb_save_single_frame(self, source_frame_path: str) -> str | None:
        ...
        if not source_frame_path or not os.path.isfile(source_frame_path):
            self.message_manager.add_error(
                "Source frame to save does not exist or is invalid."
            )
            return None
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
            dest_filename = (
                f"saved_frame_{parent_folder_name}_{timestamp}_{frame_filename}"
            )

            destination_path = os.path.join(
                self.toolbox_permanent_save_dir, dest_filename
            )
            ...
            self.message_manager.add_success(
                f"Saved frame to permanent storage: {destination_path}"
            )
            return destination_path
        except Exception as e:
            self.message_manager.add_error(
                f"Error saving frame to permanent storage: {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            return None
    ...
    def set_autosave_mode(self, autosave_enabled: bool, silent: bool = False):
        if autosave_enabled:
            ...
            if not silent:
                self.message_manager.add_message(
                    "Autosave ENABLED: Processed videos will be saved to the permanent folder.",
                    "SUCCESS",
                )
        else:
            self.toolbox_video_output_dir = self._base_temp_output_dir
            if not silent:
                self.message_manager.add_message(
                    "Autosave DISABLED: Processed videos will be saved to the temporary folder.",
                    "INFO",
                )

    def _tb_log_ffmpeg_error(
        self, e_ffmpeg: subprocess.CalledProcessError, operation_description: str
    ):
        ...
        if ffmpeg_stderr_str:
            details_log.append(f"FFmpeg Stderr: {ffmpeg_stderr_str}")
        if ffmpeg_stdout_str:
            details_log.append(f"FFmpeg Stdout: {ffmpeg_stdout_str}")
        ...
        if details_log:
            ...
            self.message_manager.add_message(
                f"No specific output from FFmpeg. (Return code: {e_ffmpeg.returncode}, Command: '{e_ffmpeg.cmd}')",
                "INFO",
            )
    ...
    def _tb_get_video_frame_count(self, video_path: str) -> int | None:
        ...
        if not self.has_ffprobe:
            self.message_manager.add_message(
                "Cannot get frame count: ffprobe not found.", "DEBUG"
            )
            return None
        ...
        try:
            ...
            if frame_count_str.isdigit() and int(frame_count_str) > 0:
                self.message_manager.add_message(
                    f"Frame count from metadata: {frame_count_str}", "DEBUG"
                )
                return int(frame_count_str)
            else:
                self.message_manager.add_warning(
                    f"Fast metadata frame count was invalid ('{frame_count_str}'). Falling back to full count."
                )
        except Exception as e:
            self.message_manager.add_warning(
                f"Fast metadata read failed: {e}. Falling back to full count."
            )

        # --- Tier 2: Slow, accurate full-stream count using JSON output ---
        try:
            self.message_manager.add_message(
                "Performing full, accurate frame count with ffprobe (this may take a moment)...",
                "INFO",
            )
            ...
            if frame_count_str.isdigit() and int(frame_count_str) > 0:
                self.message_manager.add_message(
                    f"Accurate frame count from full scan: {frame_count_str}", "DEBUG"
                )
                return int(frame_count_str)
            else:
                self.message_manager.add_error(
                    f"Full ffprobe scan returned invalid frame count: '{frame_count_str}'."
                )
                return None
        except Exception as e:
            self.message_manager.add_error(
                f"Critical error during full ffprobe frame count: {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            return None
    ...
    def tb_extract_frames(self, video_path, extraction_rate, progress=gr.Progress()):
        ...
        if not isinstance(extraction_rate, int) or extraction_rate < 1:
            self.message_manager.add_error(
                "Extraction rate must be a positive integer (1 for all frames, N for every Nth frame)."
            )
            return None

        resolved_video_path = str(Path(video_path).resolve())
        output_folder_name = self._tb_generate_output_folder_path(
            resolved_video_path, suffix=f"extracted_every_{extraction_rate}"
        )
        ...
        try:
            ...
            if total_frames:
                ...
                self.message_manager.add_warning(
                    "Could not determine total frames. Progress will be indeterminate."
                )
            ...
            for i, frame in enumerate(reader):
                ...
                if total_frames and i % 10 == 0:
                    progress(
                        i / total_frames,
                        desc=f"Extracting {i} / {total_frames} frames...",
                    )
            ...
            self.message_manager.add_success(
                f"Successfully extracted {extracted_count} frames to: {output_folder_name}"
            )
    ...
    def tb_get_extracted_frame_folders(self) -> list:
        if not os.path.exists(self.extracted_frames_target_path):
            self.message_manager.add_warning(
                f"Extracted frames directory not found: {self.extracted_frames_target_path}"
            )
            return []
        try:
            folders = [
                d
                for d in os.listdir(self.extracted_frames_target_path)
            ...
            self.message_manager.add_error(
                f"Error scanning for extracted frame folders: {e}"
            )
            return []
    ...
    def tb_delete_extracted_frames_folder(self, folder_name_to_delete: str) -> bool:
        ...
        folder_path_to_delete = os.path.join(
            self.extracted_frames_target_path, folder_name_to_delete
        )

        if not os.path.exists(folder_path_to_delete) or not os.path.isdir(
            folder_path_to_delete
        ):
            self.message_manager.add_error(
                f"Folder not found or is not a directory: {folder_path_to_delete}"
            )
            return False
        ...
        try:
            ...
            self.message_manager.add_success(
                f"Successfully deleted folder: {folder_name_to_delete}"
            )
            return True
        except Exception as e:
            self.message_manager.add_error(
                f"Error deleting folder '{folder_name_to_delete}': {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            return False

    def tb_reassemble_frames_to_video(
        self,
        frames_source,
        output_fps,
        output_base_name_override=None,
        progress=gr.Progress(),
    ):
        if not frames_source:
            self.message_manager.add_warning(
                "No frames source (folder or files) provided for reassembly."
            )
            return None
        ...
        self.message_manager.add_message(
            f"Starting frame reassembly to video at {output_fps} FPS."
        )
        ...
        try:
            ...
            if isinstance(frames_source, str) and os.path.isdir(frames_source):
                self.message_manager.add_message(
                    f"Processing frames from directory: {frames_source}"
                )
                # Use our existing function to get a sorted list of frame paths
                sorted_frame_paths = self.tb_get_frames_from_folder(
                    os.path.basename(frames_source)
                )
                ...
                self.message_manager.add_error(
                    "Invalid frames_source type for reassembly. Expected a directory path."
                )
                return None

            if not frame_info_list:
                self.message_manager.add_warning(
                    "No valid image files found in the provided source to reassemble."
                )
                return None

            self.message_manager.add_message(
                f"Found {len(frame_info_list)} frames for reassembly."
            )

            output_file_basename = "reassembled_video"
            if (
                output_base_name_override
                and isinstance(output_base_name_override, str)
                and output_base_name_override.strip()
            ):
                sanitized_name = "".join(
                    c if c.isalnum() or c in (" ", "_", "-") else "_"
                    for c in output_base_name_override.strip()
                )
                output_file_basename = Path(sanitized_name).stem
                if not output_file_basename:
                    output_file_basename = "reassembled_video"
                self.message_manager.add_message(
                    f"Using custom output video base name: {output_file_basename}"
                )
            ...
            self.message_manager.add_message(
                "Reading frame images (in sorted order)..."
            )
            ...
            for frame_info in frame_iterator:
                ...
                try:
                    if not filename_for_log.lower().endswith(
                        (".png", ".jpg", ".jpeg", ".bmp", ".webp")
                    ):
                        self.message_manager.add_warning(
                            f"Skipping non-standard image file: {filename_for_log}."
                        )
                        continue
                    frames_data.append(imageio.imread(frame_actual_path))
                except Exception as e_read_frame:
                    self.message_manager.add_warning(
                        f"Could not read frame ({filename_for_log}): {e_read_frame}. Skipping."
                    )

            if not frames_data:
                self.message_manager.add_error(
                    "No valid frames could be successfully read for reassembly."
                )
                return None

            self.message_manager.add_message(
                f"Writing {len(frames_data)} frames to video: {output_video_path}"
            )
            imageio.mimwrite(
                output_video_path,
                frames_data,
            ...
            self.message_manager.add_success(
                f"Successfully reassembled {len(frames_data)} frames into: {output_video_path}"
            )
            ...
            self.message_manager.add_error(traceback.format_exc())
            if (
                "Could not find a backend" in str(e)
                or "No such file or directory: 'ffmpeg'" in str(e).lower()
            ):
                self.message_manager.add_error(
                    "This might indicate an issue with FFmpeg backend for imageio. Ensure 'imageio-ffmpeg' is installed or FFmpeg is in PATH."
                )
            return None
        finally:
    ...
    def _tb_get_video_duration(self, video_path: str) -> str | None:
        ...
        try:
            ...
            result = subprocess.run(
                ffprobe_cmd, capture_output=True, text=True, check=True, errors="ignore"
            )
    ...
    def tb_join_videos(
        self, video_paths: list, output_base_name_override=None, progress=gr.Progress()
    ):
        if not video_paths or len(video_paths) < 2:
            self.message_manager.add_warning(
                "Please select at least two videos to join."
            )
            return None
        if not self.has_ffmpeg:
            self.message_manager.add_error(
                "FFmpeg is required for joining videos. This operation cannot proceed."
            )
            return None

        self.message_manager.add_message(
            f"🚀 Starting video join process for {len(video_paths)} videos..."
        )
        ...
        if not all(first_video_dims):
            self.message_manager.add_error(
                "Could not determine dimensions of the first video. Cannot proceed."
            )
            return None
        target_w, target_h = first_video_dims
        self.message_manager.add_message(
            f"Standardizing all videos to {target_w}x{target_h} for joining."
        )
        ...
        for i, path in enumerate(video_paths):
            ...
            if self._tb_has_audio_stream(path):
                ...
                if duration:
                    ...
                    self.message_manager.add_message(
                        f"'{Path(path).name}' has no audio. Generating silent track of {float(duration):.2f}s.",
                        "INFO",
                    )
                    filter_complex_parts.append(
                    ...
                    self.message_manager.add_warning(
                        f"Could not get duration for '{Path(path).name}' to generate silent audio. This track's audio will be skipped."
                    )
                    continue
        ...
        filter_complex_parts.append(
            f"{''.join(video_stream_labels)}concat=n={len(video_paths)}:v=1:a=0[outv]"
        )

        # Only add the audio concat filter if we successfully prepared audio streams.
        if audio_stream_labels:
            filter_complex_parts.append(
                f"{''.join(audio_stream_labels)}concat=n={len(audio_stream_labels)}:v=0:a=1[outa]"
            )
        ...
        if (
            ...
            base_name_to_use = (
                Path(sanitized_name).stem
                if sanitized_name
                else Path(video_paths[0]).stem
            )
        else:
        ...
        try:
            self.message_manager.add_message(
                "Running FFmpeg to join videos. This may take a while..."
            )
            progress(0.5, desc=f"Joining {len(video_paths)} videos...")

            subprocess.run(
                ffmpeg_cmd, check=True, capture_output=True, text=True, errors="ignore"
            )

            progress(1.0, desc="Join complete.")
            self.message_manager.add_success(
                f"✅ Videos successfully joined! Output: {output_path}"
            )
            return output_path
            ...
            self.message_manager.add_error(
                f"An unexpected error occurred during video joining: {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            return None
        finally:
    ...
    def tb_export_video(
        ...
        if not self.has_ffmpeg:
            self.message_manager.add_error(
                "FFmpeg is required for exporting. This operation cannot proceed."
            )
            return None

        self.message_manager.add_message(
            f"🚀 Starting export to {export_format.upper()}..."
        )
        ...
        if export_format == "MP4":
            ...
            self.message_manager.add_message(
                f"Exporting MP4 with CRF: {crf_value} (Quality: {quality_slider}%)"
            )
            ffmpeg_cmd.extend(
            ...
            self.message_manager.add_message(
                f"Exporting WebM with CRF: {crf_value} (Quality: {quality_slider}%)"
            )
            ffmpeg_cmd.extend(
                ["-c:v", "libvpx-vp9", "-crf", str(crf_value), "-b:v", "0"]
            )
            ffmpeg_cmd.extend(
                ["-c:a", "libopus", "-b:a", "96k"]
            )  # Use Opus for WebM audio
        ...
        if (
            ...
            base_name_to_use = (
                Path(sanitized_name).stem if sanitized_name else Path(video_path).stem
            )
        else:
        ...
        if export_format == "GIF":
            ...
            self.message_manager.add_message(
                "GIF export detected. Output will be forced to the permanent 'saved_videos' folder, ignoring Autosave setting.",
                "INFO",
            )
        else:
        ...
        try:
            ...
            if export_format == "GIF":
                self.message_manager.add_success(
                    f"✅ GIF successfully created and saved to: {output_path}"
                )
                self.message_manager.add_warning(
                    "⚠️ Note: The video player shows a re-encoded MP4 for preview. Your original GIF is in the output folder."
                )
            else:
                self.message_manager.add_success(
                    f"✅ Successfully exported to {export_format.upper()}! Output: {output_path}"
                )
    ...
    def _tb_generate_output_path(
        self, input_material_name, suffix, target_dir, ext=".mp4"
    ):
    ...
    def tb_copy_video_to_permanent_storage(self, temp_video_path):
        if not temp_video_path or not os.path.exists(temp_video_path):
            self.message_manager.add_error(
                "No video file provided or file does not exist to save."
            )
            return temp_video_path

        try:
            video_filename = Path(temp_video_path).name
            permanent_video_path = os.path.join(
                self.toolbox_permanent_save_dir, video_filename
            )
            os.makedirs(self.toolbox_permanent_save_dir, exist_ok=True)
            self.message_manager.add_message(
                f"Copying '{video_filename}' to permanent storage: '{permanent_video_path}'"
            )
            ...
            self.message_manager.add_error(
                f"Error saving video to permanent storage: {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            return temp_video_path
    ...
    def tb_analyze_video_input(self, video_path):
        ...
        if self.has_ffprobe:
            self.message_manager.add_message(
                f"Analyzing video with ffprobe: {os.path.basename(video_path)}"
            )
            try:
                probe_cmd = [
                    self.ffprobe_exe,
                ...
                if not video_stream:
                    self.message_manager.add_error(
                        "No video stream found in the file (ffprobe)."
                    )
                    ...
                    if (
                        avg_fps_val > 0 and abs(r_fps_val - avg_fps_val) > 0.01
                    ):  # Only show average if meaningfully different
                        calculated_fps = (
                            avg_fps_val  # Prefer average if it's different and valid
                        )
                    ...
                    self.message_manager.add_success(
                        "Video analysis complete (using ffprobe)."
                    )
                ...
                self.message_manager.add_warning(
                    f"ffprobe analysis failed ({type(e_ffprobe).__name__}). Trying imageio fallback."
                )
        ...
        if analysis_source == "imageio":  # Either ffprobe not available, or it failed
            self.message_manager.add_message(
                f"Analyzing video with imageio: {os.path.basename(video_path)}"
            )
            reader = None
            try:
                reader = imageio.get_reader(resolved_video_path)
                ...
                duration_display = (
                    f"{float(duration_imgio_val):.2f} seconds"
                    if duration_imgio_val is not None
                    else "N/A"
                )

                fps_val_imgio = meta.get("fps")
                fps_display = (
                    f"{float(fps_val_imgio):.2f} FPS"
                    if fps_val_imgio is not None
                    else "N/A"
                )
                ...
                if nframes_val_imgio_meta not in [
                    float("inf"),
                    "N/A",
                    None,
                ] and isinstance(nframes_val_imgio_meta, (int, float)):
                    ...
                    try:
                        ...
                        if nframes_val_imgio_count != float("inf"):
                            ...
                            nframes_display = "Unknown (Stream or very long)"
                    except Exception:
                        nframes_display = "Unknown (Frame count failed)"

                has_audio_str = "(Audio info not available via imageio)"
                self.message_manager.add_success(
                    "Video analysis complete (using imageio)."
                )
            except Exception as e_imgio:
                self.message_manager.add_error(
                    f"Error analyzing video with imageio: {e_imgio}"
                )
                import traceback

                self.message_manager.add_error(traceback.format_exc())
                return "Error analyzing video: Both ffprobe (if attempted) and imageio failed."
            finally:
                if reader:
                    reader.close()
        ...
        if video_width > 0 and video_height > 0:  # Ensure we have dimensions
            ...
            is_hd_or_larger = video_width >= HD_WIDTH_THRESHOLD or video_height >= (
                HD_WIDTH_THRESHOLD * 9 / 16 * 0.95
            )  # Adjusted height for aspect ratios
            is_4k_or_larger = video_width >= FOUR_K_WIDTH_THRESHOLD or video_height >= (
                FOUR_K_WIDTH_THRESHOLD * 9 / 16 * 0.95
            )
            ...
            if (
                num_frames_value and num_frames_value > 900
            ):  # e.g., > 30 seconds at 30fps
                upscale_warnings.append(
    ...
    def _tb_has_audio_stream(self, video_path_to_check):
        ...
        try:
            ...
            if (
                audio_check_result.returncode == 0
                and "audio" in audio_check_result.stdout.strip().lower()
            ):
                return True
            else:
            ...
            self.message_manager.add_warning(
                "FFprobe executable not found during audio stream check (should have been caught by self.has_ffprobe). Assuming no audio."
            )
            return False  # Should ideally not happen if self.has_ffprobe is true and self.ffprobe_exe is set
        except Exception as e:
            self.message_manager.add_warning(
                f"Error checking for audio stream in {os.path.basename(video_path_to_check)}: {e}. Assuming no audio."
            )
            return False

    def tb_process_frames(
        self,
        video_path,
        target_fps_mode,
        speed_factor,
        ...
        try:
            ...
            if "2x" in target_fps_mode:
                interpolation_factor = 2
            elif "4x" in target_fps_mode:
                interpolation_factor = 4
            should_interpolate = interpolation_factor > 1

            self.message_manager.add_message(
                f"Starting frame processing: FPS Mode: {target_fps_mode}, Speed: {speed_factor}x"
            )
            ...
            if use_streaming and speed_factor != 1.0:
                self.message_manager.add_warning(
                    "Speed factor is not applied in Streaming Interpolation mode. Processing at 1.0x speed."
                )
                speed_factor = 1.0

            op_suffix_parts = []
            if speed_factor != 1.0:
                op_suffix_parts.append(f"speed{speed_factor:.2f}x".replace(".", ","))
            ...
            final_muxed_output_path = video_stream_output_path.replace(
                "_temp_video", ""
            )
            ...
            if use_streaming:
                ...
                if not should_interpolate:
                    self.message_manager.add_warning(
                        "Streaming mode selected but no interpolation chosen. Writing video without changes."
                    )
                    ...
                    self.message_manager.add_message(
                        f"Attempting to load RIFE model for {interpolation_factor}x interpolation..."
                    )
                    if not self.rife_handler._ensure_model_downloaded_and_loaded():
                        self.message_manager.add_error(
                            "RIFE model could not be loaded. Aborting."
                        )
                        return None
                    ...
                    if n_frames is None:
                        self.message_manager.add_error(
                            "Cannot determine video length for streaming progress. Aborting."
                        )
                        return None
                    ...
                    try:
                        ...
                        self.message_manager.add_warning("Video has no frames.")
                        return None

                    # This list will only be used if we are doing a 4x (2-pass) interpolation
                    intermediate_frames_for_pass2 = (
                        [frame1_np] if num_passes > 1 else None
                    )
                    ...
                    for i, frame2_np in enumerate(reader, 1):
                        ...
                        middle_frame_np = self.rife_handler.interpolate_between_frames(
                            frame1_np, frame2_np
                        )
                    ...
                    if num_passes > 1 and intermediate_frames_for_pass2:
                        self.message_manager.add_message(
                            f"RIFE Pass 2/{num_passes}: Interpolating 2x frames (in-memory)..."
                        )
                        ...
                        for i in pass2_iterator:
                            ...
                            p2_middle = self.rife_handler.interpolate_between_frames(
                                p2_frame1, p2_frame2
                            )
                ...
                # --- IN-MEMORY (FAST) PATH ---
                self.message_manager.add_message(
                    "Reading all video frames into memory..."
                )
                ...
                if speed_factor != 1.0:
                    self.message_manager.add_message(
                        f"Adjusting speed by {speed_factor}x (in-memory)..."
                    )
                    if speed_factor > 1.0:
                        indices = np.arange(0, len(video_frames), speed_factor).astype(
                            int
                        )
                        processed_frames = [
                            video_frames[i] for i in indices if i < len(video_frames)
                        ]
                    else:
                        new_len = int(len(video_frames) / speed_factor)
                        indices = np.linspace(0, len(video_frames) - 1, new_len).astype(
                            int
                        )
                ...
                if should_interpolate and len(processed_frames) > 1:
                    self.message_manager.add_message(
                        f"Loading RIFE for {interpolation_factor}x interpolation (in-memory)..."
                    )
                    if not self.rife_handler._ensure_model_downloaded_and_loaded():
                        self.message_manager.add_error(
                            "RIFE model could not be loaded."
                        )
                        return None
            ...
            original_video_has_audio = (
                self._tb_has_audio_stream(resolved_video_path)
                if can_process_audio
                else False
            )

            if can_process_audio and original_video_has_audio:
                self.message_manager.add_message(
                    "Original video has audio. Processing audio with FFmpeg..."
                )
                ...
                if speed_factor != 1.0:
                    if 0.5 <= speed_factor <= 100.0:
                        ...
                        num_half_steps = int(
                            np.ceil(np.log(speed_factor) / np.log(0.5))
                        )
                        for _ in range(num_half_steps):
                            audio_filters.append("atempo=0.5")
                        final_factor = speed_factor / (0.5**num_half_steps)
                        if (
                            abs(final_factor - 1.0) > 1e-4
                            and 0.5 <= final_factor <= 100.0
                        ...
                        if (
                            abs(remaining_factor - 1.0) > 1e-4
                            and 0.5 <= remaining_factor <= 100.0
                        ):
                            audio_filters.append(f"atempo={remaining_factor:.4f}")

                    self.message_manager.add_message(
                        f"Applying audio speed adjustment with atempo: {','.join(audio_filters) if audio_filters else 'None (speed_factor out of simple atempo range or 1.0)'}"
                    )
                ...
                try:
                    subprocess.run(
                        ffmpeg_mux_cmd, check=True, capture_output=True, text=True
                    )
                    self.message_manager.add_success(
                        f"Video saved with processed audio: {final_muxed_output_path}"
                    )
                except subprocess.CalledProcessError as e_mux:
                    self._tb_log_ffmpeg_error(e_mux, "audio processing/muxing")
                    if os.path.exists(final_muxed_output_path):
                ...
                if original_video_has_audio and not can_process_audio:
                    self.message_manager.add_warning(
                        "Original video has audio, but FFmpeg is not available to process it. Output will be silent."
                    )
                if (
                    os.path.exists(final_muxed_output_path)
                    and final_muxed_output_path != video_stream_output_path
                ):
                    os.remove(final_muxed_output_path)
                os.rename(video_stream_output_path, final_muxed_output_path)

            if (
                os.path.exists(video_stream_output_path)
                and video_stream_output_path != final_muxed_output_path
            ):
                try:
                    os.remove(video_stream_output_path)
                except Exception as e_clean:
                    self.message_manager.add_warning(
                        f"Could not remove temp video file {video_stream_output_path}: {e_clean}"
                    )

            progress(1.0, desc="Complete.")
            self.message_manager.add_success(
                f"Frame processing complete: {final_output_path}"
            )
    ...
    def tb_create_loop(self, video_path, loop_type, num_loops, progress=gr.Progress()):
        if video_path is None:
            self.message_manager.add_warning("No input video for loop creation.")
            return None
        if (
            not self.has_ffmpeg
        ):  # FFmpeg is essential for this function's stream_loop and complex filter
            self.message_manager.add_error(
                "FFmpeg is required for creating video loops. This operation cannot proceed."
            )
            return video_path  # Return original video path
        if loop_type == "none":
            self.message_manager.add_message("Loop type 'none'. No action.")
            return video_path
        ...
        self.message_manager.add_message(
            f"Creating {loop_type} ({num_loops}x) for {os.path.basename(resolved_video_path)}..."
        )

        ping_pong_unit_path = None
        original_video_has_audio = self._tb_has_audio_stream(
            resolved_video_path
        )  # Check once
        ...
        try:
            ...
            if loop_type == "ping-pong":
                ...
                ffmpeg_pp_unit_cmd = [
                    ...
                    resolved_video_path,
                    "-vf",
                    "split[main][tmp];[tmp]reverse[rev];[main][rev]concat=n=2:v=1:a=0",  # Video only
                    "-an",
                    ping_pong_unit_path,
                ]
                subprocess.run(
                    ffmpeg_pp_unit_cmd, check=True, capture_output=True, text=True
                )
                self.message_manager.add_message(
                    f"Created ping-pong unit (video-only): {ping_pong_unit_path}"
                )
                ...
                ffmpeg_cmd = [
                    ...
                    "error",
                    "-stream_loop",
                    str(num_loops - 1),  # Loop the video unit N-1 times (total N plays)
                    "-i",
                    ping_pong_unit_path,
                ]
                ...
                if original_video_has_audio:
                    ...
                    ffmpeg_cmd.extend(
                        [
                            "-i",
                            resolved_video_path,  # Original video for audio source
                            "-filter_complex",
                            f"[1:a]areverse[areva];[1:a][areva]concat=n=2:v=0:a=1[ppa];[ppa]aloop=loop={num_loops - 1}:size=2147483647[a_looped]",
                            "-map",
                            "0:v:0",
                            "-map",
                            "[a_looped]",
                    ...
                    self.message_manager.add_message(
                        "No audio in original or detection issue. Creating video-only ping-pong loop."
                    )
                ...
                if ffmpeg_stream_loop_value < 0:
                    ffmpeg_stream_loop_value = (
                        0  # Should ideally not be hit if slider min is 1.
                    )
                ...
                ffmpeg_cmd = [
                    ...
                    "error",
                    "-stream_loop",
                    str(ffmpeg_stream_loop_value),  # This now uses num_loops directly
                    "-i",
                    resolved_video_path,
                    "-c:v",
                    "copy",
                ...
                if original_video_has_audio:
                    self.message_manager.add_message(
                        "Original video has audio. Re-encoding to AAC for looped MP4 (if not already AAC)."
                    )
                    ffmpeg_cmd.extend(
                        [
                            "-c:a",
                    ...
                    self.message_manager.add_message(
                        "No audio in original or detection issue. Looped video will be silent."
                    )
                    ffmpeg_cmd.extend(["-an", "-map", "0:v:0"])
                ffmpeg_cmd.append(output_path)

            self.message_manager.add_message(
                f"Processing video {loop_type} with FFmpeg..."
            )
            ...
            if ping_pong_unit_path and os.path.exists(ping_pong_unit_path):
                try:
                    ...
                    self.message_manager.add_warning(
                        f"Could not remove temp ping-pong unit: {e_clean_pp}"
                    )
            gc.collect()
    ...
    def _tb_get_video_dimensions(self, video_path):
        ...
        if self.has_ffprobe:
            try:
                ...
                self.message_manager.add_warning(
                    f"ffprobe failed to get dimensions ({e_probe_dim}), trying imageio."
                )
        ...
        try:
            ...
            self.message_manager.add_warning(
                f"Error getting video dimensions for vignette (imageio): {e_meta}. Defaulting aspect to 1/1."
            )
        finally:
            if reader:
                reader.close()
    ...
    def _tb_create_vignette_filter(self, strength_percent, width, height):
        ...
        angle_rad = min_angle_rad + normalized_strength * (
    ...
    def tb_apply_filters(
        ...
        if video_path is None:
            self.message_manager.add_warning("No input video for filters.")
            return None
        if not self.has_ffmpeg:  # FFmpeg is essential for this function
            self.message_manager.add_error(
                "FFmpeg is required for applying video filters. This operation cannot proceed."
            )
            return video_path
        ...
        resolved_video_path = str(Path(video_path).resolve())
        output_path = self._tb_generate_output_path(
            resolved_video_path, "filtered", self.toolbox_video_output_dir
        )
        self.message_manager.add_message(
            f"🎨 Applying filters to {os.path.basename(resolved_video_path)}..."
        )

        video_width, video_height = 0, 0
        if vignette > 0:  # Only get dimensions if vignette is used
            video_width, video_height = self._tb_get_video_dimensions(
                resolved_video_path
            )
            if video_width > 0 and video_height > 0:
                self.message_manager.add_message(
                    f"Video dimensions for vignette: {video_width}x{video_height}",
                    "DEBUG",
                )
        ...
        if denoise > 0:
            filters.append(
                f"hqdn3d={denoise * 0.8:.1f}:{denoise * 0.6:.1f}:{denoise * 0.7:.1f}:{denoise * 0.5:.1f}"
            )
            applied_filter_descriptions.append("Denoise (hqdn3d)")
        if temperature != 0:
            mid_shift = (temperature / 100.0) * 0.3
            filters.append(f"colorbalance=rm={mid_shift:.2f}:bm={-mid_shift:.2f}")
            applied_filter_descriptions.append("Color Temp")
        eq_parts = []
        desc_eq = []
        if brightness != 0:
            eq_parts.append(f"brightness={brightness / 100.0:.2f}")
            desc_eq.append("Brightness")
        if contrast != 1:
            eq_parts.append(f"contrast={contrast:.2f}")
            desc_eq.append("Contrast (Linear)")
        if saturation != 1:
            eq_parts.append(f"saturation={saturation:.2f}")
            desc_eq.append("Saturation")
        if eq_parts:
        ...
        if s_curve_contrast > 0:
            ...
            y1 = 0.25 - s * (0.25 - 0.10)
            y2 = 0.75 + s * (0.90 - 0.75)
            filters.append(f"curves=all='0/0 0.25/{y1:.2f} 0.75/{y2:.2f} 1/1'")
            applied_filter_descriptions.append("S-Curve Contrast")
        if blur > 0:
            filters.append(f"gblur=sigma={blur * 0.4:.1f}")
            applied_filter_descriptions.append("Blur")
        if sharpen > 0:
            filters.append(
                f"unsharp=luma_msize_x=5:luma_msize_y=5:luma_amount={sharpen * 0.3:.2f}"
            )
            applied_filter_descriptions.append("Sharpen")
        if film_grain_strength > 0:
            filters.append(f"noise=alls={film_grain_strength * 0.5:.1f}:allf=t+u")
            applied_filter_descriptions.append("Film Grain")
        if vignette > 0:
            filters.append(
                self._tb_create_vignette_filter(vignette, video_width, video_height)
            )
            applied_filter_descriptions.append("Vignette")

        # --- CORRECTED LOGIC ---
        if applied_filter_descriptions:
            self.message_manager.add_message(
                "🔧 Applying FFmpeg filters: " + ", ".join(applied_filter_descriptions)
            )
        else:
            self.message_manager.add_message(
                "ℹ️ No filters selected. Passing video through (re-encoding)."
            )
        ...
        try:
            ...
            if original_video_has_audio:
                self.message_manager.add_message(
                    "Original video has audio. Re-encoding to AAC for filtered video.",
                    "INFO",
                )
                ffmpeg_cmd.extend(["-c:a", "aac", "-b:a", "192k", "-map", "0:a:0?"])
            else:
                self.message_manager.add_message(
                    "No audio in original or detection issue. Filtered video will be silent.",
                    "INFO",
                )
            ...
            self.message_manager.add_success(
                f"✅ Filter step complete! Output: {output_path}"
            )
            return output_path
    ...
    def tb_upscale_video(
        self,
        video_path,
        model_key: str,
        output_scale_factor_ui: float,
        tile_size: int,
        enhance_face: bool,
        ...
        try:
            ...
            if model_key not in self.esrgan_upscaler.supported_models:
                self.message_manager.add_error(
                    f"Upscale model key '{model_key}' not found in supported models."
                )
                return None

            model_native_scale = self.esrgan_upscaler.supported_models[model_key].get(
                "scale", 0
            )
            ...
            upsampler_instance = self.esrgan_upscaler.load_model(
                model_key=model_key,
                tile_size=tile_size,
                denoise_strength=denoise_strength_ui
                if model_key == "RealESR-general-x4v3"
                else None,
            )
            if not upsampler_instance:
                self.message_manager.add_error(
                    f"Could not load ESRGAN model '{model_key}'. Aborting."
                )
                return None

            if enhance_face:
                if not self.esrgan_upscaler._load_face_enhancer(
                    bg_upsampler=upsampler_instance
                ):
                    self.message_manager.add_warning(
                        "GFPGAN load failed. Proceeding without face enhancement."
                    )
                    enhance_face = False

            self.message_manager.add_message(
                f"ESRGAN model '{model_key}' loaded. Initializing process..."
            )
            ...
            temp_video_suffix_base = (
                f"upscaled_{model_key}{'_FaceEnhance' if enhance_face else ''}"
            )
            if model_key == "RealESR-general-x4v3" and denoise_strength_ui is not None:
                temp_video_suffix_base += f"_dni{denoise_strength_ui:.2f}"
            temp_video_suffix = temp_video_suffix_base.replace(".", "p") + "_temp_video"
            video_stream_output_path = self._tb_generate_output_path(
                resolved_video_path, temp_video_suffix, self.toolbox_video_output_dir
            )
            final_muxed_output_path = video_stream_output_path.replace(
                "_temp_video", ""
            )
            ...
            if use_streaming:
                ...
                if n_frames is None:
                    self.message_manager.add_error(
                        "Cannot use streaming mode because the total number of frames could not be determined. Aborting."
                    )
                    return None
                ...
                for i in progress.tqdm(
                    range(n_frames), desc="Upscaling Frames (Streaming)"
                ):
                    frame_np = reader.get_data(i)  # Explicitly get frame i

                    upscaled_frame_np = self.esrgan_upscaler.upscale_frame(
                        frame_np, model_key, float(output_scale_factor_ui), enhance_face
                    )
                    if upscaled_frame_np is not None:
                        writer.append_data(upscaled_frame_np)
                    else:
                        self.message_manager.add_error(
                            f"Failed to upscale frame {i}. Skipping."
                        )
                        if (
                            "out of memory"
                            in self.message_manager.get_recent_errors_as_str(
                                count=1
                            ).lower()
                        ):
                            self.message_manager.add_error(
                                "CUDA OOM. Aborting video upscale."
                            )
                            return None
                ...
                all_frames = [
                    frame for frame in progress.tqdm(reader, desc="Reading all frames")
                ]
                upscaled_frames = []
                frame_iterator = progress.tqdm(
                    all_frames, desc="Upscaling Frames (In-Memory)"
                )
            ...
            reader = None
            self.message_manager.add_message(
                f"Upscaled video stream saved to: {video_stream_output_path}"
            )
            ...
            if can_process_audio and original_video_has_audio:
                ...
                self.message_manager.add_message(
                    "Original video has audio. Muxing audio with FFmpeg..."
                )
                ffmpeg_mux_cmd = [
                    self.ffmpeg_exe,
                    "-y",
                ...
                try:
                    ...
                    self.message_manager.add_success(
                        f"Upscaled video saved with audio: {final_muxed_output_path}"
                    )
                except subprocess.CalledProcessError as e_mux:
                ...
                if original_video_has_audio and not can_process_audio:
                    self.message_manager.add_warning(
                        "Original video has audio, but FFmpeg is not available to process it. Upscaled output will be silent."
                    )
                if (
                    os.path.exists(final_muxed_output_path)
                    and final_muxed_output_path != video_stream_output_path
            ...
            self.message_manager.add_success(
                f"Video upscaling complete: {final_output_path}"
            )
            return final_output_path
            ...
            if (
                video_stream_output_path
                and os.path.exists(video_stream_output_path)
                and final_output_path
                and video_stream_output_path != final_output_path
            ):
                try:
                    os.remove(video_stream_output_path)
                except Exception as e_clean:
                    self.message_manager.add_warning(
                        f"Could not remove temp upscaled video: {e_clean}"
                    )
    ...
    def tb_open_output_folder(self):
        ...
        try:
            ...
            self.message_manager.add_success(
                f"Opened postprocessed output folder: {folder_path}"
            )
        except Exception as e:
    ...
    def _tb_clean_directory(self, dir_path, dir_description):
        ...
        try:
            display_path = (
                os.path.relpath(dir_path, self.project_root) if dir_path else "N/A"
            )
        ...
        try:
            ...
            file_count = sum(
                1 for item in items if os.path.isfile(os.path.join(dir_path, item))
            )
            dir_count = sum(
                1 for item in items if os.path.isdir(os.path.join(dir_path, item))
            )
            ...
            if file_count == 0 and dir_count == 0:
                ...
                status_text = (
                    f"Cleaned ({' and '.join(summary_parts)}) from '{display_path}'"
                )
    ...
    def tb_clear_temporary_files(self):
        ...
        postproc_summary_line = self._tb_clean_directory(
            postproc_temp_dir, "Post-processing temp folder"
        )

        # 2. Clean Gradio Temp Folder
        gradio_temp_dir = self.settings.get("gradio_temp_dir")
        gradio_summary_line = self._tb_clean_directory(
            gradio_temp_dir, "Gradio temp folder"
        )
