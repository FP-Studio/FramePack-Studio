...
class ESRGANUpscaler:
    def __init__(self, message_manager: MessageManager, device: torch.device):
        ...
        os.makedirs(
            self.gfpgan_model_dir, exist_ok=True
        )  # Ensure GFPGAN model dir exists
    ...
    def _ensure_model_downloaded(
        ...
        is_gfpgan: bool = False,
        is_wdn_companion: bool = False,
    ) -> bool:
        ...
        if is_gfpgan:
            ...
            if model_key not in self.supported_models:
                self.message_manager.add_error(
                    f"ESRGAN model key '{model_key}' not supported."
                )
                return False
            ...
            if is_wdn_companion:
                if (
                    "wdn_filename" not in model_details
                    or "wdn_file_url" not in model_details
                ):
                    self.message_manager.add_error(
                        f"WDN companion model details missing for '{model_key}'."
                    )
                    return False
                model_info_source = {
        ...
        if not model_path.exists():
            ...
            self.message_manager.add_message(
                f"{log_prefix}Model '{actual_model_filename}' not found. Downloading..."
            )
            try:
                ...
                if "file_url" in model_info_source and model_info_source["file_url"]:
                    ...
                    for url in urls_to_try:
                        self.message_manager.add_message(
                            f"Attempting download from URL: {url}"
                        )
                        try:
                            load_file_from_url(
                                url=url,
                                ...
                                progress=True,
                                file_name=actual_model_filename,
                            )
                            if model_path.exists():
                                downloaded_successfully = True
                                self.message_manager.add_success(
                                    f"{log_prefix}Model '{actual_model_filename}' downloaded from URL."
                                )
                                break
                        except Exception as e_url:
                            self.message_manager.add_warning(
                                f"Failed to download from {url}: {e_url}. Trying next source."
                            )
                            continue

                if (
                    not downloaded_successfully
                    and "hf_repo_id" in model_info_source
                    and model_info_source["hf_repo_id"]
                ):
                    self.message_manager.add_message(
                        f"Attempting download from Hugging Face Hub: {model_info_source['hf_repo_id']}"
                    )
                    snapshot_download(
                        repo_id=model_info_source["hf_repo_id"],
                        allow_patterns=[actual_model_filename],
                        local_dir=current_model_dir,
                        local_dir_use_symlinks=False,
                    )
                    if model_path.exists():
                        downloaded_successfully = True
                        self.message_manager.add_success(
                            f"{log_prefix}Model '{actual_model_filename}' downloaded from Hugging Face Hub."
                        )

                if not downloaded_successfully:
                    self.message_manager.add_error(
                        f"All download attempts failed for '{actual_model_filename}'."
                    )
                    return False
            except Exception as e:
                self.message_manager.add_error(
                    f"Failed to download {log_prefix}model '{actual_model_filename}': {e}"
                )
    ...
    def load_model(
        self, model_key: str, tile_size: int = 0, denoise_strength: float | None = None
    ) -> RealESRGANer | None:
        if model_key not in self.supported_models:
            self.message_manager.add_error(
                f"ESRGAN model key '{model_key}' not supported."
        ...
        current_config_signature = (
            tile_size,
            denoise_strength if model_key == "RealESR-general-x4v3" else None,
        )
        ...
        if model_key in self.upsamplers:
            ...
            existing_config_signature = (
                ...
                existing_config.get("denoise_strength")
                if model_key == "RealESR-general-x4v3"
                else None,
            )

            if (
                existing_config.get("upsampler") is not None
                and existing_config_signature == current_config_signature
            ):
                log_tile = f"Tile: {str(tile_size) if tile_size > 0 else 'Auto'}"
                log_dni = (
                    f", DNI: {denoise_strength:.2f}"
                    if denoise_strength is not None
                    and model_key == "RealESR-general-x4v3"
                    else ""
                )
                self.message_manager.add_message(
                    f"ESRGAN model '{model_key}' ({log_tile}{log_dni}) already loaded."
                )
        ...
        if (
            ...
            and denoise_strength is not None
            and 0.0 <= denoise_strength < 1.0
        ):
            ...
            if "wdn_filename" not in model_info or "wdn_file_url" not in model_info:
                self.message_manager.add_error(
                    f"WDN companion model details missing for '{model_key}'. Cannot apply denoise strength."
                )
            ...
            if not self._ensure_model_downloaded(model_key, is_wdn_companion=True):
                self.message_manager.add_error(
                    f"Failed to download WDN companion for '{model_key}'. Cannot apply denoise strength."
                )
                return None

            wdn_model_path_str = str(self.model_dir / model_info["wdn_filename"])
            model_path_for_upsampler = [
                model_path_for_upsampler,
                wdn_model_path_str,
            ]  # Pass list of paths
            dni_weight_for_upsampler = [
                denoise_strength,
                1.0 - denoise_strength,
            ]  # [main_model_strength, wdn_model_strength]
        ...
        try:
            ...
            elif "upscale" in model_params_with_correct_scale:
            ...
            self.message_manager.add_success(
                f"ESRGAN model '{model_info['filename']}' (Key: {model_key}) loaded successfully."
            )
            return upsampler
        except Exception as e:
            self.message_manager.add_error(
                f"Failed to load ESRGAN model '{model_info['filename']}' (Key: {model_key}): {e}"
            )
            self.message_manager.add_error(traceback.format_exc())
            if model_key in self.upsamplers:
                del self.upsamplers[model_key]
            return None

    def _load_face_enhancer(
        self, model_name="GFPGANv1.4.pth", bg_upsampler=None
    ) -> bool:
        if not GFPGAN_AVAILABLE:
            self.message_manager.add_warning(
                "GFPGAN library not available. Cannot load face enhancer."
            )
            return False
        if self.face_enhancer is not None:
            # If bg_upsampler changed, we might need to re-init. For now, assume if loaded, it's fine or will be handled by caller.
            if (
                bg_upsampler is not None
                and hasattr(self.face_enhancer, "bg_upsampler")
                and self.face_enhancer.bg_upsampler != bg_upsampler
            ):
                self.message_manager.add_message(
                    "GFPGAN face enhancer already loaded, but with a different background upsampler. Re-initializing GFPGAN..."
                )
        ...
        if not self._ensure_model_downloaded(model_key=model_name, is_gfpgan=True):
            self.message_manager.add_error(
                f"Failed to download GFPGAN model '{model_name}'."
            )
            return False

        gfpgan_model_path = str(self.gfpgan_model_dir / model_name)
        self.message_manager.add_message(
            f"Loading GFPGAN face enhancer from {gfpgan_model_path}..."
        )
        try:
    ...
    def unload_model(self, model_key: str):
        if (
            model_key in self.upsamplers
            and self.upsamplers[model_key].get("upsampler") is not None
        ):
            config = self.upsamplers.pop(model_key)
            upsampler_instance = config["upsampler"]
            tile_s = config.get("tile_size", 0)
            native_scale = config.get(
                "native_scale", "N/A"
            )  # Get native_scale for logging
            log_tile_size = str(tile_s) if tile_s > 0 else "Auto"
            self.message_manager.add_message(
                f"Unloading ESRGAN model '{model_key}' (Scale: {native_scale}x, Tile: {log_tile_size})..."
            )

            if (
                self.face_enhancer
                and hasattr(self.face_enhancer, "bg_upsampler")
                and self.face_enhancer.bg_upsampler == upsampler_instance
            ):
                self.message_manager.add_message(
                    "Unloading associated GFPGAN as its BG upsampler is being removed."
                )
            ...
            gc.collect()
            self.message_manager.add_success(
                f"ESRGAN model '{model_key}' unloaded and memory cleared."
            )
        else:
            self.message_manager.add_message(
                f"ESRGAN model '{model_key}' not loaded, no need to unload."
            )

    def unload_all_models(self):
        if not self.upsamplers and not self.face_enhancer:
            self.message_manager.add_message(
                "No ESRGAN or GFPGAN models currently loaded."
            )
            return
        ...
        gc.collect()
        self.message_manager.add_success(
            "All ESRGAN and GFPGAN models unloaded and memory cleared."
        )
    ...
    def upscale_frame(
        ...
        if config and config.get("upsampler"):
            ...
            if model_native_scale == 0:
                self.message_manager.add_error(
                    f"Error: Native scale for model '{model_key}' is 0 or not found in config."
                )
                return None
        ...
        if upsampler is None:
            ...
            if upsampler is None:
                self.message_manager.add_error(
                    f"Failed to auto-load ESRGAN model '{model_key}'. Cannot upscale."
                )
                return None
            ...
            if loaded_config:
                ...
                if model_native_scale == 0:
                    self.message_manager.add_error(
                        f"Error: Native scale for auto-loaded model '{model_key}' is 0."
                    )
                    return None
            else:
                self.message_manager.add_error(
                    f"Error: Config for auto-loaded model '{model_key}' not found."
                )
                return None
        ...
        if enhance_face:
            if not self.face_enhancer or (
                hasattr(self.face_enhancer, "bg_upsampler")
                and self.face_enhancer.bg_upsampler != upsampler
            ):
                self.message_manager.add_message(
                    "Face enhancement requested, loading/re-configuring GFPGAN..."
                )
                self._load_face_enhancer(bg_upsampler=upsampler)

            if not self.face_enhancer:
                self.message_manager.add_warning(
                    "GFPGAN could not be loaded. Proceeding without face enhancement."
                )
                enhance_face = False
        ...
        try:
            ...
            if enhance_face and self.face_enhancer:
                if (
                    self.face_enhancer.upscale != 1
                ):  # Ensure GFPGAN is only cleaning, not upscaling itself in this pipeline path
                    self.message_manager.add_warning(
                ...
                _, _, cleaned_img_bgr = self.face_enhancer.enhance(
                    img_bgr, has_aligned=False, only_center_face=False, paste_back=True
                )
                output_bgr, _ = upsampler.enhance(
                    cleaned_img_bgr, outscale=outscale_for_enhance
                )
            else:
                output_bgr, _ = upsampler.enhance(
                    img_bgr, outscale=outscale_for_enhance
                )
            ...
            tile_size_msg_part = (
                str(current_tile_size) if current_tile_size > 0 else "Auto"
            )
