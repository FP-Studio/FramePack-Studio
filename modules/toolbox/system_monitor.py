...
class SystemMonitor:
    ...
    def get_nvidia_gpu_info() -> Tuple[str, MetricsDict, Optional[str]]:
        ...
        warning_message = (
            None  # To indicate if nvidia-smi failed and we're using PyTorch fallback
        )
        ...
        if smi_output_str:
            ...
            if (
                len(parts) == 5
            ):  # memory.used, memory.total, memory.reserved, temp, util
                memory_used_mib, memory_total_mib, memory_reserved_mib, temp, util = (
                    map(float, parts)
                )
                metrics = {
                    "memory_used_gb": memory_used_mib / 1024,
                    "memory_total_gb": memory_total_mib / 1024,
                    "memory_reserved_gb": memory_reserved_mib
                    / 1024,  # This is from nvidia-smi's memory.reserved
                    "temperature": temp,
                    "utilization": util,
                }
            else:
    ...
    @staticmethod
    def get_mac_gpu_info() -> Tuple[
        str, MetricsDict, Optional[str]
    ]:  # Add warning return for consistency
        ...
        try:
            ...
            metrics = {
                ...
                / (1024**3),  # This is system RAM, reported as "Unified Memory"
            ...
            ):  # Check if it's actually CPU util
    ...
    def get_amd_gpu_info() -> Tuple[
        ...
        try:
            ...
            try:
                    ...
                    if line.startswith("GPU[0]"):  # Assuming card 0
                        if "VRAM Usage:" in line:
                            ...
                            metrics["memory_total_gb"] = (
                                float(mem_parts[1].replace("M", "")) / 1024
                            )
                            source = "rocm-smi"
                        elif "Temperature:" in line:
                            metrics["temperature"] = float(
                                line.split("Temperature:")[1].strip().replace("c", "")
                            )
                            source = "rocm-smi"
                        elif "GPU Use:" in line:
                            metrics["utilization"] = float(
                                line.split("GPU Use:")[1].strip().replace("%", "")
                            )
                            source = "rocm-smi"
                ...
                if platform.system() == "Linux":
                    ...
                    try:
                        with open(
                            f"{base_path}/hwmon/hwmon0/temp1_input"
                        ) as f:  # Check for specific hwmon index
                            metrics["temperature"] = float(f.read().strip()) / 1000
                        sysfs_found_any = True
                    except (FileNotFoundError, PermissionError, ValueError):
                        pass  # Ignore if specific file not found

                    try:
                        with open(f"{base_path}/mem_info_vram_total") as f:
                            metrics["memory_total_gb"] = int(f.read().strip()) / (
                                1024**3
                            )  # Bytes to GiB
                        with open(f"{base_path}/mem_info_vram_used") as f:
                            metrics["memory_used_gb"] = int(f.read().strip()) / (
                                1024**3
                            )  # Bytes to GiB
                        sysfs_found_any = True
                    except (FileNotFoundError, PermissionError, ValueError):
        ...
        except Exception:  # Catch-all for unforeseen issues in AMD block
            # print(f"General error in get_amd_gpu_info: {e_amd_main}")
            warning_message = (
                warning_message or ""
            ) + " Unexpected error in AMD GPU info gathering."
    ...
    def get_system_info(cls) -> str:
        ...
        try:
            ...
            if gpu_name_display:
                ...
                if (
                    "temperature" in metrics and metrics.get("temperature", 0.0) > 0
                ):  # Only show if temp is valid
                    gpu_info_lines.append(
                        f"🌡️ GPU Temp: {metrics.get('temperature', 0.0):.0f}°C"
                    )

                if "utilization" in metrics:
                    gpu_info_lines.append(
                        f"⚡ GPU Load: {metrics.get('utilization', 0.0):.0f}%"
                    )
            ...
            cpu_info = (
                f"💻 CPU: {cpu_count or 'N/A'} Cores, {cpu_threads or 'N/A'} Threads\n"
            )
