...
def get_changed_settings_pills(job_data: dict) -> str:
    ...
    always_show = {'model_type', 'generation_type', 'resolutionW', 'resolutionH', 'seed', 'prompt'}
    
    # Settings that should not be shown as pills (internal/system fields)
    never_show = {
        'prompt', 'negative_prompt', 'seed', 'end_frame_image_path', 
        'input_video', 'video_path', 'x_param', 'y_param', 'x_values', 'y_values'
    }
    ...
    for key, value in job_data.items():
        if key not in always_show and key not in never_show:
            ...
            if isinstance(value, float):
                ...
                if lora_count > 0:
                    formatted_value = f"{lora_count} LoRA{'s' if lora_count != 1 else ''}"
                else:
                    continue
            else:
